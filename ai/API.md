...
- 제출 결과 조회 API는 학생이 제출한 코드를 `submittedCode` 필드로 반환한다.
- 문제세트 결과 조회와 관리자 학생 풀이 기록 조회도 주관식 답안 필드 없이 `submittedCode`를 반환한다.
- 이미 정답 처리된 문제에 다시 제출하면 `409 SUB-006`을 반환한다.
- 프론트는 문제 상태가 `CORRECT`이면 제출 버튼을 비활성화하고, 기존 제출 코드는 `submittedCode`로 표시한다.
- 챗봇 FastAPI 요청의 외부 JSON 키 `submitted_answer`는 기존 연동 호환성을 위해 유지하지만, 백엔드 내부 모델명은 `submittedCode`를 사용한다.
- 현재 문제 유형은 코드 실행형만 사용하며 주관식 제출 API와 응답 필드는 제공하지 않는다.

| 필드 | 타입 | 설명 |
|------|------|------|
| `submittedCode` | String | 학생이 제출한 코드 |
...
문제 데이터셋 실행 흐름에는 Signed URL 전달과 Runner 임시 파일 실행을 적용했다. 다만 시작 코드·일부 API 응답의 URL 노출과 실제 버킷 공개 권한, Cloud Run 호출 인증은 추가 보강이 필요하다.

| 항목 | 현재 | 남은 보강 |
|------|------|-----------|
| CSV 데이터셋 실행 | 30분 Signed URL을 Runner에 별도 전달하고 임시 로컬 파일로 실행 | Private bucket 운영 확인 및 시작 코드 URL 노출 제거 |
| 뱃지 이미지 | GCS 이미지 저장 어댑터 미구현 | Private bucket + Signed URL 또는 인증 프록시 |
| API 응답 | 일부 데이터셋 URL 문자열 반환 | `url`, `expiresAt` 또는 `fileKey` 포함 여부 확정 |
| Runner 호출 | HTTP 연결·응답 타임아웃 및 장애 로그 적용 | Cloud Run 비공개 전환 및 ID Token 인증 |
| 보상 삭제 | DB 저장 실패 시 GCS 객체 삭제 | 객체 삭제 실패 모니터링 보강 |

### 데이터셋 실행 보안 정책

- Signed URL 만료 시간: `30분`
- Signed URL 전달 대상: Spring 서버에서 Cloud Run Python Runner로만 별도 전달
- Runner 처리: HTTPS GCS URL만 허용하고 임시 디렉터리에 CSV를 다운로드한 뒤 실행 종료 시 삭제
- 다운로드 제한: 최대 `50MB`
- 사용자 코드 실행 제한: 기본 `5000ms`, 환경변수 `CODE_EXECUTION_DEFAULT_TIMEOUT_MS`로 변경 가능
- 설정 허용 범위: `100~10000ms`
- HTTP 호출 제한: 연결 기본 `3000ms`, 응답 대기 기본 `15000ms`
...
- `PUT /api/v1/problems/{problemSetId}/with-dataset`: `multipart/form-data`의 `request` JSON 파트와 선택적 `datasetFile` CSV 파트를 받는다.
  - 프론트는 `request`를 `application/json` 타입의 Blob으로 전달하고 전체 `Content-Type` 헤더는 브라우저가 boundary와 함께 생성하도록 둔다.
  - 데이터셋 파일을 새로 전달하면 기존 활성 데이터셋은 `INACTIVE` 처리하고 새 데이터셋을 `ACTIVE`로 저장한다.
...
- 문제세트 등록·수정 요청은 문제와 중첩 테스트케이스 필수값 및 실행 제한 시간을 Controller 단계에서 검증한다.
- 검증 실패 시 `400 COMMON-BAD-REQUEST` 응답이 반환될 수 있으며 정상 성공 응답 구조는 유지한다.
- 운영자 수정 조회의 테스트케이스와 첫 번째 힌트는 문제 ID 목록 기준으로 일괄 조회하며 응답 구조는 변경하지 않는다.
