...
> 컨벤션(§0~§3)은 **강제**다. 지금부터 안 지키면 나중에 팀 통합 대시보드가 불가능해진다.

---

# 0. 로컬 측정의 의미 경계 (강제 해석 규칙)

로컬 k6 결과는 **두 부류로 나눠 해석한다. 섞으면 안 된다.** 노트북마다 하드웨어가 달라서, 무엇을 합격 기준으로 쓰고 무엇을 상대 신호로만 볼지가 갈린다.

## 0.1 환경 독립 — 하드 게이트 (PR 합격 기준)

어느 기계서나 동일하게 재현되는 것 → 로컬로 판정해도 유효:

- `http_req_failed < 1%` (4xx/5xx)
- 모든 `check` 통과
- 데드락 / 커넥션풀 고갈 / 메모리 누수 없음

→ **"내 코드가 가상 트래픽에서 버티나"의 판정. 로컬로 충분, EC2 불필요.**

## 0.2 환경 종속 — 상대 신호만 (로컬 하드 게이트 금지)

하드웨어에 따라 값이 달라지는 것:

- 절대 `http_req_duration` p95/p99 (latency 절대값)

→ **같은 기계의 before/after % 만 유효.**
- ⛔ 팀원 간 절대값 비교 금지 ("나는 200ms 나왔는데" = 네 노트북 성능)
- ⛔ "우리 시스템 p95 = Xms" 시스템 수용치로 인용 금지
- ✅ 절대 수용치(동시 몇 명 · p95 몇 ms)는 **고정 환경 1곳(배포 EC2)에서만** 측정·인용

> 한 줄 요약: **"버티나(O/X)" = 로컬 / "얼마나(수용 천장·절대 p95)" = 고정 환경 1곳.**

## 0.3 실패모드 전이 보장 — config 핀

로컬 회복력 검증(§0.1)이 배포에서도 유효하려면, 동시성 한계를 결정하는 config를 **배포 EC2와 동일하게 둔다:**

- Hikari `maximum-pool-size`
- Tomcat `max-threads`
- 타임아웃 (connect/read, async request-timeout)

config가 환경마다 다르면 "로컬서 풀 고갈" ≠ "EC2서 풀 고갈"이 되어 전이가 깨진다. **프로파일별로 다르게 덮어쓰지 않는다**(공통 설정에 두거나, 둘 다 같은 값).

> **구체 수치는 배포 스펙이 단일 소스다.** 배포 타깃(Spring t3.small / RDS db.t3.micro)에 맞춘 값을 쓰고, 로컬도 그 값을 그대로 쓴다.
> ⚠️ **병목을 인위로 만들려고 풀을 줄이지 마라.** 실제 배포할 값을 그대로 쓰고, 그 값이 목표 부하(30 VU)에서 버티는지를 본다. 안 터지면 "병목 없음"이 정당한 결과다.

## 0.4 VU는 목적이 정한다 (숫자를 찍지 마라)

| 목적 | VU | 비고 |
|------|-----|------|
| 코드 직렬 지연 % 측정 | **1 VU** | 워밍업 후, 다운스트림 목, p50(med) 비교 |
| 병목 "위치" 탐색 | **ramp** | 무릎(RPS 평평 + latency 꺾임 + 에러 시작) 찾기 |
| before/after 비교 | **동일 VU** | 다른 VU끼리 비교 = 무의미 |

> latency % 개선을 보려면 1 VU여야 깨끗하다 — 50 VU는 경합 노이즈(노트북 코어 수 의존)가 껴서 % 가 흔들린다. 부하형(ramp) 시나리오는 병목 "위치"를 찾는 용도지 코드 % 측정용이 아니다.

> **구체 stages·VU 레시피(smoke / code-perf / baseline / stress / worst-case)는 [`LOAD-PROFILES.md`](LOAD-PROFILES.md)가 단일 소스다.** 여기(§0.4)는 원칙, 거기는 복붙용 프로파일.
...
> - promtail이 `level`·`log_type`을 JSON에서 뽑아 **Loki 라벨**로 만든다(`{job="lms", level="ERROR"}`). 이미 `monitoring-local/promtail/promtail-config.yml`에 적용됨 — 평문으로 되돌리지 말 것.
...
| 산출물 | 위치 |
|--------|------|
| 도메인별 모니터링 산출물(문서) | `monitoring-local/docs/domains/<domain>/` |
| k6 공통 lib | `monitoring-local/k6/lib/` |
| k6 도메인 시나리오 | `monitoring-local/k6/scripts/<domain>/*.js` |
| k6 결과 | `monitoring-local/k6/results/` |

> k6는 도커 스택(`monitoring-local/docker-compose.yml`)의 `k6` 컨테이너로 실행한다. `lib`/`scripts`/`results`가 그대로 컨테이너에 마운트된다. 실행법은 `monitoring-local/README.md`.
...
공통 기본값(로컬):
```text
- http_req_failed < 1%                  ← 하드 게이트 (환경 독립, §0.1)
- http_req_duration p95 < 1000ms        ← 참고선(advisory). 환경 종속이라 로컬 합격/불합격 기준 아님(§0.2). 절대 수용 판정은 EC2.
...
> job 라벨은 **`lms`** 다(promtail `monitoring-local/promtail/promtail-config.yml`). 로그는 JSON이라 line filter는 raw 라인에 먹고, 숫자/필드 추출은 `regexp`/`json`을 쓴다.
...
> **우리 스택은 k6 메트릭도 Prometheus로 보낸다** — k6 컨테이너를 `-o experimental-prometheus-rw`로 실행(`monitoring-local/docker-compose.yml`, Prometheus `--web.enable-remote-write-receiver`). 그래서 교안과 달리 **Grafana에서 k6 클라이언트 p95·실패율(`k6_http_req_duration_*` 등)까지** 서버측 메트릭과 같은 화면에서 본다.
> 단 k6 콘솔·`monitoring-local/k6/results/*.md` 요약도 그대로 남으니 전후 비교는 그 파일로 한다.
