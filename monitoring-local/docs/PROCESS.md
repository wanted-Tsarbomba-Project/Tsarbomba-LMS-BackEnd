...
| # | 항목 | 내용 | 현재 상태 |
|---|------|------|-----------|
| 0-1 | 모니터링 스택 | `monitoring-local/docker-compose.yml`: Prometheus + Loki + Promtail + Grafana **+ loadtest MySQL(3307) + k6 컨테이너**. prometheus scrape(`host.docker.internal:8080/actuator/prometheus`), grafana datasource provisioning, k6는 `-o experimental-prometheus-rw`로 메트릭까지 전송 | ✅ **완료**(이미 동작) |
...
| 0-7 | k6 공통 lib | `monitoring-local/k6/lib/`: `config.js`(BASE_URL 등) · `summary.js`(handleSummary md/json) · `auth.js`(쿠키 기반 로그인 헬퍼 `login()`/`authCookies()`) | ✅ **완료** — ⚠️ loadtest DB에 로그인 가능한 시드 계정 필요(5단계) |

> **JWT 헬퍼가 교안에 없던 핵심 항목.** 우리 API는 대부분 `@PreAuthorize` 인증 필수다. k6에서 로그인→토큰 발급→`Authorization` 헤더를 모든 요청에 싣는 공통 헬퍼가 없으면 인증 API 부하테스트 자체가 불가능하다. (현재 `monitoring-local/k6/scripts/auth/`에 인증 스크립트가 시작돼 있음 — 공통 `auth.js`로 추출 권장)
>
...
`monitoring-local/k6/lib/` 공통 헬퍼(+`auth.js` JWT)를 import해서 `monitoring-local/k6/scripts/<domain>/*.js` 작성. baseline 최소 1 + 시나리오 1. 실행은 도커 k6 컨테이너(`monitoring-local/README.md` 참조).
...
- 제약: 로컬, MySQL, 단일 앱, **JWT 인증 필요(monitoring-local/k6/lib/auth.js 사용)**
...
>
> 🚨 **반드시 `loadtest` 프로파일로 앱 실행**(`--spring.profiles.active=loadtest`). local로 부하 주면 운영 DB(RDS)로 트래픽이 가서 과금/장애 위험. (`monitoring-local/README.md` 경고 참조)
...
각 팀원은 아래를 `monitoring-local/docs/domains/<domain>/`에 제출하면 "완료"다.

```text
monitoring-local/docs/domains/<domain>/
...
monitoring-local/k6/scripts/<domain>/*.js   # 4단계: 부하 시나리오 (코드)
monitoring-local/k6/results/*-summary.md    # 5·7단계: k6 raw 결과(before/after) — handleSummary 자동 저장
```

> k6 raw 요약(`*-summary.md`)은 `monitoring-local/k6/results/`에 자동 저장된다. 도메인 문서 폴더의 `compare.md`는 그중 핵심값(p95·waiting·실패율·custom duration·EXPLAIN)만 전후로 정리한 발표용 표다.
